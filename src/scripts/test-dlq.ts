...
async function runTest() {
  ...
  console.log(`🗑️ Immediately deleting ${invalidDocId} from S3 to trigger a NoSuchKey exception...`);
  ...
  const validResult = await docClient.send(new QueryCommand({
    TableName: TABLE_NAME,
    KeyConditionExpression: 'PK = :pk',
    ExpressionAttributeValues: { ':pk': `DOCUMENT#${validDocId}` }
  }));

  const invalidResult = await docClient.send(new QueryCommand({
    TableName: TABLE_NAME,
    KeyConditionExpression: 'PK = :pk',
  ...
  console.log('\n======================================================');
  console.log(`Valid Document Ingested:   ${validProcessed ? '✅ YES' : '❌ NO'}`);
  console.log(`Invalid Document Ingested: ${invalidProcessed ? '❌ YES (Should have failed)' : '✅ NO'}`);
  console.log('======================================================');

  if (validProcessed && !invalidProcessed) {
    console.log('🎉 TEST SUCCESSFUL! SQS successfully isolated the failure.');
    console.log('   - The valid document was processed and written to DynamoDB.');
    console.log('   - The missing S3 document threw a NoSuchKey error, reported a batch item failure, and was correctly isolated from the successful message.');
  } else {
