...
| **Database Cost Ceiling** | **TIP** DynamoDB table configured with provisioned capacity (**1 RCU / 1 WCU base, 1 RCU / 1 WCU index** by default; configurable via stack parameters). | Acts as a budget boundary, preventing database scaling costs from skyrocketing during attacks. |
